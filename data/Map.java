...
public class Map {
	...
	private float wymiar_x;
	private float wymiar_y;
	 ...
	 * - pola wymiar_x oraz wymiar_y musz¹ zawieraæ liczby z przedzia³u (0,50> (w metrach)
	...
	public float getWymiarX() {
		return wymiar_x;
	}
	
	public void setWymiarX(float wymiar_x_mapy) throws MapException{
		if((wymiar_x_mapy <= 0) || (wymiar_x_mapy > 50))
			throw new MapException("Wymiary musz¹ byc w przedziale (0,50>.");
	...
	public void setWymiarX(String wymiar_x_mapy) throws MapException {
		...
		try { 
			setSkala(Integer.parseInt(wymiar_x_mapy));
		} catch (NumberFormatException e) {
			throw new MapException("Wymiary musza byæ liczba wymierna");
		}
	}
	
	public float getWymiarY() {
		return wymiar_y;
	}
	
	public void setWymiarY(float wymiar_y_mapy) throws MapException{
		if((wymiar_y_mapy <= 0) || (wymiar_y_mapy > 50))
			throw new MapException("Wymiary musz¹ byc w przedziale (0,50>.");
	...
	public void setWymiarY(String wymiar_y_mapy) throws MapException {
		...
		try { 
			setSkala(Integer.parseInt(wymiar_y_mapy));
		} catch (NumberFormatException e) {
			throw new MapException("Wymiary musza byæ liczba wymierna");
		}
	}
